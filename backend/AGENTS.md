...
The backend defaults to Firebase Firestore (`PRINTX_STORAGE=firestore`). It lazily initializes its server-only service-account credential on the first request, so Next.js/Vercel builds do not need production credentials. It accepts `FIREBASE_SERVICE_ACCOUNT_FILE`, `FIREBASE_SERVICE_ACCOUNT_JSON`, or `FIREBASE_SERVICE_ACCOUNT_BASE64`; use JSON/base64 in Vercel rather than a local filesystem path, and never send these values to the browser. The first Firestore adapter stores the structured MVP state in the `printx/state` document and migrates an existing `PRINTX_DATA_FILE` once when that document is empty. `PRINTX_STORAGE=local` is an explicit local-only fallback for development.
