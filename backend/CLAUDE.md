...
Use pnpm (`pnpm@9.15.9`) for all workspace commands. The backend defaults to Firebase Firestore for structured application persistence; server-only Firebase service-account credentials must stay out of frontend code and version control. Use `PRINTX_STORAGE=local` only for an intentional local-only development run.
